...
const points = [
  ...
  { label: 'Build-ready planning', icon: ClipboardCheck },
]
  ...
  return (
    <section className="bg-brand-bg py-16 text-brand-charcoal sm:py-24" aria-labelledby="house-land-preview-title">
      <div className="section-shell grid gap-10 lg:grid-cols-[0.95fr_1.05fr] lg:items-center">
        <motion.div
          className="relative overflow-hidden border border-brand-border bg-brand-surface p-3 shadow-premium rounded-2xl"
        ...
        >
          <div className="relative overflow-hidden rounded-xl">
            ...
            <div className="absolute bottom-5 left-5 right-5 border-l border-brand-gold/70 pl-4">
              <p className="text-[10px] font-semibold uppercase tracking-[0.24em] text-gold-soft">
                Sydney &amp; New South Wales
              </p>
              <p className="mt-2 font-display text-2xl font-normal text-white">Premium residential opportunities</p>
            </div>
          </div>
        </motion.div>
        ...
        >
          <p className="eyebrow text-brand-gold">Residential pathways</p>
          <h2 id="house-land-preview-title" className="mt-4 font-display text-3xl font-normal leading-tight text-brand-charcoal sm:text-4xl">
            House &amp; Land Packages
          </h2>
          <p className="mt-5 max-w-2xl text-base font-light leading-8 text-brand-muted">
            Explore premium house and land package opportunities across Sydney and New South Wales, supported by practical construction guidance, development knowledge, and trusted project delivery.
          </p>
          ...
          <div className="mt-8 grid gap-3 sm:grid-cols-2">
            {points.map(({ label, icon: Icon }, index) => (
              <motion.div
                key={label}
                className="flex items-center gap-3 border border-brand-border bg-brand-surface px-4 py-4 rounded-lg shadow-sm"
              ...
              >
                ...
                <span className="text-xs font-bold uppercase tracking-[0.14em] text-brand-charcoal">{label}</span>
              </motion.div>
            ))}
          </div>
